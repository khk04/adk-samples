...
데이터 분석 및 리포트 생성 시스템 - CSV와 Excel 형식의 데이터를 분석하여 한국어 리포트를 자동 생성하고 품질을 평가합니다.

## 📌 프로젝트 개요

이 프로젝트는 Google ADK(Agent Development Kit)를 기반으로 한 데이터 분석 및 리포트 생성 시스템입니다. CSV와 Excel 형식의 데이터 파일을 업로드하면 자동으로 형식을 감지하고 분석하여 한국어 비즈니스 리포트를 생성하며, 4가지 기준으로 품질을 평가하여 임계값에 도달할 때까지 반복 개선합니다.

## 🏗️ 아키텍처

### 에이전트 구조
- **LoopAgent**: `data_report_generator` - 메인 루프 에이전트
- **SequentialAgent**: `data_analysis_report_evaluation_agent` - 데이터 분석, 리포트 생성 및 평가 순차 실행
- **Data Analysis Report Agent**: CSV와 Excel 형식의 데이터 분석 및 리포트 생성
- **Report Evaluation Agent**: 리포트 품질 평가
- **Checker Agent**: 루프 종료 조건 확인

### 워크플로우
1. **Artifact 파일 감지**: Artifact에 업로드된 파일을 자동으로 감지하고 data 디렉토리에 저장
2. **데이터 형식 감지**: 업로드된 파일의 형식을 자동으로 감지 (CSV, Excel)
3. **데이터 분석**: 감지된 형식에 맞는 파서를 사용하여 데이터 분석
4. **리포트 생성**: 분석된 데이터를 바탕으로 한국어 리포트 생성
5. **품질 평가**: 4가지 기준으로 리포트 품질 평가 (0-10점)
6. **조건 확인**: 품질 점수가 임계값(8.0점) 이상이거나 최대 반복 횟수(3회) 도달 시 종료
7. **반복 개선**: 조건 미충족 시 1-6단계 반복
...
### 1. 데이터 분석
- **주요 형식 지원**: CSV와 Excel 파일 자동 감지 및 분석
- **Artifact 통합**: Artifact에 업로드된 파일을 자동으로 감지하고 data 디렉토리에 저장
- **자동 형식 감지**: 파일 확장자와 내용을 기반으로 데이터 형식 자동 감지
- **통계 분석**: 숫자형 데이터의 기본 통계, 카테고리별 분석, 상관관계 분석
- **시계열 분석**: 날짜 데이터가 있는 경우 시간별 트렌드 분석
- **pandas 기반 데이터 처리**: 모든 형식을 DataFrame으로 통일하여 처리

### 2. 자동 리포트 생성
- **CSV/Excel 데이터 지원**: 각 형식의 특성을 고려한 맞춤형 분석
- **한국어 비즈니스 리포트**: 전문적이고 명확한 한국어 리포트 자동 생성
- **구조화된 리포트 형식**: 요약, 주요 발견사항, 데이터 분석, 결론 및 권장사항
- **데이터 기반 인사이트**: 통계적 분석을 바탕으로 한 실용적인 인사이트 제공
...
3. **메모리 사용량**: 대용량 파일의 경우 메모리 사용량에 주의하세요.
