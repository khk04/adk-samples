...
class ReportGenerator:
    ...
    def generate_report(self, content: str, template_name: str = "default") -> str:
        """리포트 생성"""
        try:
            logger.info(f"리포트 생성 시작: {template_name}")
            
            # 간단한 템플릿 기반 리포트 생성
            report = self._apply_template(content, template_name)
            
            logger.info(f"리포트 생성 완료: {template_name}")
            return report
...
"""
    ...
    def save_report(self, report: str, filename: str) -> str:
        ...
        try:
            ...
            logger.error(f"리포트 저장 실패: {str(e)}")
            raise